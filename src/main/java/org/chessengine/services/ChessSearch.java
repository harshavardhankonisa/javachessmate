...
public class ChessSearch {
    ...
    public static  String search(long startTime) {
        ...
        while(System.currentTimeMillis() - startTime < maxTime) {
            try {
                ChessSearch.treeConstruction(startTime,ChessboardState.WhiteToMove,depth +1, root, ChessboardState.WP, ChessboardState.WN, ChessboardState.WB, ChessboardState.WR, ChessboardState.WQ, ChessboardState.WK, ChessboardState.BP, ChessboardState.BN, ChessboardState.BB, ChessboardState.BR, ChessboardState.BQ, ChessboardState.BK, ChessboardState.EP, ChessboardState.CWK, ChessboardState.CWQ, ChessboardState.CBK, ChessboardState.CBQ);
            } catch (Exception e) {}

            try {
                ChessSearch.AlphaBeta(root, alpha, beta, depth, true, startTime);
            } catch (Exception e) {}
    ...
    public static void treeConstruction(long startTime, boolean whiteToMove, int depth, TreeNode root, long WP,long WN,long WB,long WR,long WQ,long WK,long BP,long BN,long BB,long BR,long BQ,long BK,long EP, boolean CWK, boolean CWQ, boolean CBK, boolean CBQ) throws Exception {
        ...
        for(int i=0; i<moves.length(); i+=4){
            ...
            if (((WK2 & Moves.unsafeForWhite(WP2, WN2, WB2, WR2, WQ2, WK2, BP2, BN2, BB2, BR2, BQ2, BK2)) == 0 && whiteToMove) ||
                ...
                newNode = new TreeNode(move,BoardEvaluator.boardEvaluation(checkedScore,whiteToMove,WP2,WN2,WB2,WR2,WQ2,WK2,BP2,BN2,BB2,BR2,BQ2,BK2));
    ...
    public static int AlphaBeta(TreeNode node, int alpha, int beta, int depth, boolean isMaximizing, long startTime) throws Exception {
        int value;
